...
//hashCode count
 const std::string 
   Data::getHash() const{
    const std::string str = readString(m_content);
    long long length =  str.length();
    printf("str=='HELLO WORLD'?%d",str=="HELLO WORLD");
    unsigned char md[33] = {0};  
    SHA256((const unsigned char *)str.data(), str.length(), md);  
    int i = 0;  
    char buf[65] = {0};  
    char tmp[3] = {0};  
    for(i = 0; i < 32; i++ )  
