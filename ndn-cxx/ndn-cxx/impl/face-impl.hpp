...
public: // consumer
  ...
  satisfyPendingInterests(const Data& data)
  {
    ...
    m_pendingInterestTable.removeIf([&] (PendingInterest& entry) {
      ...
      if(entry.getInterest()->getHashCode() != "" && !entry.getInterest()->matchesData(data,data.getHash())){
        std::cout << "not match pit by hashCode" << std::endl;
        return false;
      }
      else if (entry.getInterest()->getHashCode() == "" &&!entry.getInterest()->matchesData(data,"")) {
        std::cout << "not match pit by name" << std::endl;
        return false;
      }
      NDN_LOG_DEBUG("   satisfying " << *entry.getInterest() << " from " << entry.getOrigin());
   //   std::cout << " face-impl: satisfying " << *entry.getInterest() << " from " << entry.getOrigin() << std::endl;
