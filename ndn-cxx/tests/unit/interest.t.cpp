...
0x34,0x5b,0xfc,0x94,0xc7,0x58,0x94,0xed,0xd3//hashCode of "hello world" 32*4 bit
  };

  Interest i1("/local/ndn/prefix");
  i1.setCanBePrefix(true);
  i1.setNonce(1);
  i1.setHashCode("315f5bdb76d078c43b8ac0064e4a0164612b1fce77c869345bfc94c75894edd3");
...
BOOST_CHECK_EQUAL(i2.getHashCode(),"315f5bdb76d078c43b8ac0064e4a0164612b1fce77c869345bfc94c75894edd3");
...
0x34,0x5b,0xfc,0x94,0xc7,0x58,0x94,0xed,0xd3,//hashCode of "hello world" 32*4 bit
  ...
  i1.setHashCode("315f5bdb76d078c43b8ac0064e4a0164612b1fce77c869345bfc94c75894edd3");
  printf("Hi,it's come to here!");
  Block wire1 = i1.wireEncode();
//printf("\n wire1 = %s\n",readString(wire1));
printf("wire1.size:%d,WIRE.size:%d\n",wire1.size(),sizeof(WIRE));
  BOOST_CHECK_EQUAL_COLLECTIONS(wire1.begin(), wire1.end(), WIRE, WIRE + sizeof(WIRE));
...
const uint8_t WIRE[] = {//length=89,in TLV 
    0x05, 0x59, // Interest(type = 5, length = 55+32+2=89
          0x07, 0x14, // Name=7(begin)(1)(type=7,length=14)
                0x08, 0x05, 0x6c, 0x6f, 0x63, 0x61, 0x6c, // GenericNameComponent=8(2)(type=8)
                0x08, 0x03, 0x6e, 0x64, 0x6e, // GenericNameComponent
                0x08, 0x06, 0x70, 0x72, 0x65, 0x66, 0x69, 0x78, // GenericNameComponent
          0x21, 0x00, // CanBePrefix=33
          0x12, 0x00, // MustBeFresh=18
          0x1e, 0x0b, // ForwardingHint=30
                0x1f, 0x09, // Delegation List(31)
                      0x1e, 0x02,
                            0x3e, 0x15,
                      0x07, 0x03,
                            0x08, 0x01, 0x48,
          0x0a, 0x04,//Nonce=10(type = 10,length=4)
                0x4a, 0xcb, 0x1e, 0x4c, 
          0x0c, 0x02,// Interest Lifetime=12
                0x76, 0xa1, 
          0x24, 0x04, // ApplicationParameters=36

                0xc0, 0xc1, 0xc2, 0xc3,
0x0b,0x20,
0xd3,0xed,0x94,0x58,0xc7,0x94,0xfc,0x5b,0x34,0x69,0xc8,0x77,0xce,0x1f,0x2b,0x61,0x64,0x01,0x4a,
0x4e,0x06,0xc0,0x8a,0x3b,0xc4,0x78,0xd0,0x76,0xdb,0x5b,0x5f,0x31};
  Interest i1;
  i1.setName("/local/ndn/prefix");
  ...
  Block wire1 = i1.wireEncode();
 BOOST_CHECK_EQUAL_COLLECTIONS(wire1.begin(), wire1.end(), WIRE, WIRE + sizeof(WIRE));
 printf("wire1.tostring:%s\n",readString(wire1).c_str());
 printf("WIRE.tostring:%s\n",WIRE);
  Interest i2(wire1);
  BOOST_CHECK_EQUAL(i2.getName(), "/local/ndn/prefix");
  BOOST_CHECK_EQUAL(i2.getHashCode(),"315f5bdb76d078c43b8ac0064e4a0164612b1fce77c869345bfc94c75894edd3");
...
BOOST_AUTO_TEST_CASE(MatchesData)
{
  ...
  interest.setName("ndn:/A")
          ...
          .setHashCode("315f5bdb76d078c43b8ac0064e4a0164612b1fce77c869345bfc94c75894edd3");
  ...
  data.wireEncode();
  BOOST_CHECK_EQUAL(interest.matchesData(data,data.getHash()), true);//important!!!

  Data data1 = data;
  data1.setName("ndn:/A"); // violates MinSuffixComponents
  data1.wireEncode();
  BOOST_CHECK_EQUAL(interest.matchesData(data1,data1.getHash()), false);

  Interest interest1 = interest;
  interest1.setMinSuffixComponents(1);
  BOOST_CHECK_EQUAL(interest1.matchesData(data1,data1.getHash()), true);
