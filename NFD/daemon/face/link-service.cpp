...
LinkService::sendInterest(const Interest& interest)
{
  ...
  ++this->nOutInterests;
printf("------------------NFD::LinkService::sendInterest---------------------\n");
...
LinkService::sendData(const Data& data)
{
  ...
  ++this->nOutData;
  std::cout <<"LinkService:sendData data = " << data.getName() << std::endl;
  doSendData(data);
}
...
LinkService::sendNack(const ndn::lp::Nack& nack)
{
  ...
  ++this->nOutNacks;
printf("------------------NFD::LinkService::sendNACK---------------------\n");
  doSendNack(nack);
}
...
LinkService::receiveInterest(const Interest& interest)
{
  ...
  ++this->nInInterests;
printf("------------------NFD::LinkService::receiveInterest---------------------\n");
...
LinkService::receiveData(const Data& data)
{
  ...
  ++this->nInData;
    std::cout <<"LinkService:receiveData data = " << data.getName() << std::endl;
...
LinkService::receiveNack(const ndn::lp::Nack& nack)
{
  ...
  ++this->nInNacks;
printf("------------------NFD::LinkService::receiveNACK---------------------\n");
