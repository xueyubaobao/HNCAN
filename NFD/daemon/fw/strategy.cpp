...
{
  NFD_LOG_TRACE("beforeSatisfyInterest pitEntry=" << pitEntry->getName()
...
Strategy::afterContentStoreHit(const shared_ptr<pit::Entry>& pitEntry,
                               const FaceEndpoint& ingress, const Data& data)
{
  NFD_LOG_TRACE("afterContentStoreHit pitEntry=" << pitEntry->getName()
                << " in=" << ingress << " data=" << data.getName());
...
Strategy::afterReceiveData(const shared_ptr<pit::Entry>& pitEntry,
                           const FaceEndpoint& ingress, const Data& data)
{
  NFD_LOG_TRACE("afterReceiveData pitEntry=" << pitEntry->getName()
                << " in=" << ingress << " data=" << data.getName());
  std::cout <<"afterReceiveData pitEntry=" << pitEntry->getName()
                << " in=" << ingress << " data=" << data.getName()<<std::endl;
...
{
  NFD_LOG_TRACE("afterReceiveNack in=" << ingress << " pitEntry=" << pitEntry->getName());
}

void
Strategy::onDroppedInterest(const FaceEndpoint& egress, const Interest& interest)
{
  NFD_LOG_TRACE("onDroppedInterest out=" << egress << " name=" << interest.getName());
}
...
{
  BOOST_ASSERT(pitEntry->getInterest().matchesData(data,data->getHash()));
