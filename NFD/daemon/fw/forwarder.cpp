...
printf("----------------------------receive an Interest:isviolatingLocalhost--------------------\n");
...
printf("----------------------------receive an Interest:reaching-producer-region--------------------\n");
    NFD_LOG_DEBUG("onIncomingInterest in=" << ingress
...
printf("----------------------------receive an Interest:pit insert complete--------------------\n");
...
{
  NFD_LOG_TRACE("onContentStoreMiss interest=" << interest.getName());
  ++m_counters.nCsMisses;
printf("----------------------------onContentStoreMiss--------------------\n");
  // insert in-record
  ...
  if (nextHopTag != nullptr) {
    ...
    if (nextHopFace != nullptr) {
      NFD_LOG_TRACE("onContentStoreMiss interest=" << interest.getName() << " nexthop-faceid=" << nextHopFace->getId());
...
Forwarder::onIncomingData(const FaceEndpoint& ingress, const Data& data)
{
  ...
  if (isViolatingLocalhost) {
    printf("----------------------------onIncomingData:violate localhost--------------------\n");
    NFD_LOG_DEBUG("onIncomingData in=" << ingress << " data=" << data.getName() << " violates /localhost");
    // (drop)
    return;
  }
    
  // PIT match(if the hashcode is new)
    pit::DataMatchResult pitMatches;
    pitMatches = m_pit.findAllDataMatches(data);//according to names
    if (pitMatches.size() == 0) {
          //goto Data unsolicited pipeline
        printf("----------------------------onIncomingData:match size = 0, go to data unsolicited pipeline----------\n");
  ...
  if (pitMatches.size() == 1) {
    printf("----------------------------onIncomingData:only an entry is matched--------------------\n");
  ...
  else if(pitMatches.size() >= 1) {
    printf("----------------------------onIncomingData:matches over 1--------------------\n");
...
{
  std::cout<<"onOutgoingData data=" << data.getName()<<std::endl;
  if (egress.face.getId() == face::INVALID_FACEID) {
    NFD_LOG_WARN("onOutgoingData out=(invalid) data=" << data.getName());
    std::cout<<"onOutgoingData out=(invalid) data=" << data.getName()<<std::endl;
    return;
  }
  ...
  if (isViolatingLocalhost) {
   ...
   std::cout<<"onOutgoingData out=" << egress << " data=" << data.getName() << " violates /localhost"<<std::endl;
    // (drop)
    return;
  }
